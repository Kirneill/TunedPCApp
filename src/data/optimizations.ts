import type { OptimizationItem } from '../types';
import { GAMES } from './game-registry';

export const windowsOptimizations: OptimizationItem[] = [
  {
    id: 'win-power-plan',
    label: 'Ultimate Performance Power Plan',
    description: 'Intel CPUs: activates Ultimate Performance to prevent clock throttling. AMD/Ryzen CPUs keep their existing power plan.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: true,
  },
  {
    id: 'win-hags',
    label: 'Hardware-Accelerated GPU Scheduling',
    description: 'Reduces CPU overhead for GPU command scheduling. Best for RTX 30/40 and RDNA 2/3 GPUs.',
    category: 'windows',
    risk: 'moderate',
    requiresReboot: true,
  },
  {
    id: 'win-game-mode',
    label: 'Game Mode & Disable Game Bar',
    description: 'Enables Game Mode for CPU priority, disables Game Bar/DVR recording overlay to free resources.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: false,
  },
  {
    id: 'win-mmcss',
    label: 'MMCSS Game Priority',
    description: 'Allocates 90% of CPU cycles to games (SystemResponsiveness=10) and sets high GPU priority.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: false,
  },
  {
    id: 'win-network',
    label: 'Network: Disable Nagle\'s Algorithm',
    description: 'Sets TcpAckFrequency=1 and TCPNoDelay=1. Reduces network latency by 10-200ms for online games.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: true,
  },
  {
    id: 'win-visual-fx',
    label: 'Visual Effects: Best Performance',
    description: 'Disables Windows animations, transparency, and visual effects to free GPU/CPU resources.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: false,
  },
  {
    id: 'win-fullscreen',
    label: 'Disable Fullscreen Optimizations',
    description: 'Prevents Windows from intercepting fullscreen exclusive mode. Reduces input latency.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: false,
  },
  {
    id: 'win-mouse',
    label: 'Disable Mouse Acceleration',
    description: 'Sets MouseSpeed=0, disables thresholds. Critical for consistent aim and muscle memory.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: false,
  },
  {
    id: 'win-cpu-power',
    label: 'CPU Power States (No Downclocking)',
    description: 'Intel CPUs: sets minimum CPU state to 100% and disables USB suspend. All CPUs: disables hibernation.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: false,
  },
  {
    id: 'win-bg-apps',
    label: 'Disable Background Apps',
    description: 'Stops UWP/Store apps from running background tasks (sync, updates, telemetry) that steal CPU time during gaming.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: false,
  },
  {
    id: 'win-mpo',
    label: 'Disable Multiplane Overlay (MPO)',
    description: 'Fixes micro-stutters and black flickers caused by DWM multi-plane composition, especially on multi-monitor setups.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: true,
  },
  {
    id: 'win-visual-extras',
    label: 'Visual Effects Extras',
    description: 'Reduces menu delay to 200ms, minimizes keyboard repeat delay, hides Chat/TaskView icons, shrinks Search to icon.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: false,
  },
  {
    id: 'win-copilot',
    label: 'Disable Windows Copilot',
    description: 'Disables Copilot via policy, removes the appx package, and disables Bing Search in Start Menu. May affect apps that use Copilot/Bing AI features (Edge sidebar, Search AI).',
    category: 'windows',
    risk: 'moderate',
    requiresReboot: true,
  },
  {
    id: 'win-standard',
    label: 'Standard Windows Settings',
    description: 'Applies the standard baseline (privacy, services, and maintenance tasks) in one pass.',
    category: 'windows',
    risk: 'moderate',
    requiresReboot: true,
  },
  {
    id: 'win-gpu-profile',
    label: 'GPU Driver Profile (NVIDIA)',
    description: 'Imports a competitive NVIDIA profile (Power Mode Max Performance, Texture Filtering High Performance, V-Sync Off).',
    category: 'windows',
    risk: 'moderate',
    requiresReboot: false,
  },
  {
    id: 'win-dns',
    label: 'DNS Optimization',
    description: 'Switches to Cloudflare + Google DNS servers for faster server lookups. Helps with quicker matchmaking and lobby connections.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: false,
  },
  {
    id: 'win-net-adapter',
    label: 'Network Adapter Tuning',
    description: 'Configures your network card for lowest latency by disabling power-saving and packet-batching features that add delay.',
    category: 'windows',
    risk: 'moderate',
    requiresReboot: false,
  },
  {
    id: 'win-tcp-stack',
    label: 'TCP Stack Optimization',
    description: 'Fine-tunes how Windows handles network traffic so game data is processed faster with less overhead per packet.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: false,
  },
  {
    id: 'win-net-throttle',
    label: 'Network Throttling & QoS',
    description: 'Removes Windows bandwidth limits and reserved network capacity so 100% of your connection is available for gaming.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: true,
  },
  {
    id: 'win-timer-res',
    label: 'Timer Resolution (0.5ms)',
    description: 'Enables global timer resolution requests so games achieve 0.5ms tick rate. Improves 1%/0.1% lows by 20-30%.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: false,
  },
  {
    id: 'win-power-throttle',
    label: 'Disable Power Throttling',
    description: 'Prevents Windows from reducing CPU frequency for efficiency mode. Eliminates micro-stutters from incorrect thread throttling.',
    category: 'windows',
    risk: 'safe',
    requiresReboot: true,
  },
  {
    id: 'win-priority-sep',
    label: 'Foreground Priority Boost',
    description: 'Ensures foreground priority is set to the optimal 3:1 CPU ratio (restores default if changed by third-party tools or group policy).',
    category: 'windows',
    risk: 'safe',
    requiresReboot: false,
  },
  {
    id: 'win-dynamic-tick',
    label: 'Disable Dynamic Tick',
    description: 'Forces consistent system clock rate instead of power-saving dynamic ticks. Reduces latency jitter but hardware-dependent.',
    category: 'windows',
    risk: 'moderate',
    requiresReboot: true,
  },
  {
    id: 'win-hpet',
    label: 'Disable HPET',
    description: 'Ensures Windows uses the fast TSC timer instead of the slower HPET. Most modern systems benefit but results vary by hardware.',
    category: 'windows',
    risk: 'moderate',
    requiresReboot: true,
  },
];

// Derived from the unified game registry -- no manual sync needed
export const gameOptimizations: OptimizationItem[] = GAMES.map(g => ({
  id: `game-${g.id}`,
  label: g.name,
  description: g.description,
  category: 'game' as const,
  risk: g.risk,
  requiresReboot: g.requiresReboot,
  gameId: g.id,
}));

export const allOptimizations = [...windowsOptimizations, ...gameOptimizations];
